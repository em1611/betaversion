...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/em1611/betaversion/archive/main.zip"
        dir_name = "betaversion-main"
    else:
        zip_url = "https://github.com/em1611/betaversion/archive/main.zip"
    ...
    if success:
        mesgdcrt.SuccessMessage("was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update TBomb.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From bomb last version beta")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/em1611/betaversion.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get(
        "https://raw.githubusercontent.com/em1611/betaversion/main/.version"
    ).text.strip()
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("is up-to-date")
...
def notifyen():
    try:
        if DEBUG_MODE:
            url = "https://github.com/em1611/betaversion/raw/.notify"
        else:
            url = "https://github.com/em1611/betaversion/raw/master/.notify"
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("Bomb was created by ishwarSethi")
...
def workernode(mode, cc, target, count, delay, max_threads):
    ...
    print("\n")
    mesgdcrt.SuccessMessage("completed!")
    time.sleep(1.5)
    bann_text()
    sys.exit()
...
def selectnode(mode="sms"):
    ...
    try:
        ...
        max_limit = {"sms": 500000000, "call": 500000000, "mail": 500000000}
        cc, target = "", ""
        if mode in ["sms", "call"]:
            cc, target = get_phone_info()
            if cc != "91":
                max_limit.update({"sms": 500000000})
        elif mode == "mail":
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("will work only in Python v3")
    sys.exit()
...
description = """ Creating by ishwarSethi bomber script in beta testing"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by ishwarSethi !!!')
